...
class QConvLayer():
    ...
    def qforward(self, inputs, xs):
        ...
        z = (tf.nn.conv2d(input=inputs, filters=self.qw, strides=self.strides, padding=self.padding) + self.qb) * (qws * qxs)

        # descobre escala da saída com base em uma média
        self.output_scale = 0.99 * self.output_scale + 0.01 * tf.reduce_max(z) # REMOVIDO ABS DO REDUCE_MAX PQ DEPOIS VEM RELU
        ...
        # quantiza saída
        qz = quantize(z, stochastic_round=True, stochastic_zero=True)
        #################################################
