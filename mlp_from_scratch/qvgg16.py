...
def VGG_16(x, num_classes=10, **kwargs):
    ...
    # Block 2
    x = tf.keras.layers.Conv2D(128, (3, 3), padding='same', kernel_initializer="glorot_normal")(x)
    x = tf.keras.layers.ReLU()(x)
    x = tf.keras.layers.Conv2D(128, (3, 3), padding='same', kernel_initializer="glorot_normal")(x)
    x = tf.keras.layers.ReLU()(x)
    x = tf.keras.layers.MaxPooling2D((2, 2), strides=(2, 2))(x)
    ...
    # Classification head
    x = tf.keras.layers.Flatten()(x)    
    x = tf.keras.layers.Dense(256, kernel_initializer="glorot_normal")(x)
    x = tf.keras.layers.ReLU()(x)
    x = tf.keras.layers.Dense(256, kernel_initializer="glorot_normal")(x)
    x = tf.keras.layers.ReLU()(x)
    x = tf.keras.layers.Dense(num_classes, kernel_initializer="glorot_normal")(x)  # Assuming 1000 classes for ImageNet
